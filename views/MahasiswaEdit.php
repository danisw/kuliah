...
loadjs.ready("head", function () {
    ...
    fmahasiswaedit = currentForm = new ew.Form("fmahasiswaedit", "edit");
    ...
    fmahasiswaedit.addFields([
        ["id_mhs", [fields.id_mhs.visible && fields.id_mhs.required ? ew.Validators.required(fields.id_mhs.caption) : null], fields.id_mhs.isInvalid],
    ...
    // Validate form
    fmahasiswaedit.validate = function () {
    ...
    // Form_CustomValidate
    fmahasiswaedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fmahasiswaedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fmahasiswaedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fmahasiswaedit" id="fmahasiswaedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->id_mhs->Visible) { // id_mhs ?>
    <div id="r_id_mhs" class="form-group row">
        <label id="elh_mahasiswa_id_mhs" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id_mhs->caption() ?><?= $Page->id_mhs->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id_mhs->cellAttributes() ?>>
<?php if (!$Page->isConfirm()) { ?>
<span id="el_mahasiswa_id_mhs">
<span<?= $Page->id_mhs->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id_mhs->getDisplayValue($Page->id_mhs->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="mahasiswa" data-field="x_id_mhs" data-hidden="1" name="x_id_mhs" id="x_id_mhs" value="<?= HtmlEncode($Page->id_mhs->CurrentValue) ?>">
<?php } else { ?>
<span id="el_mahasiswa_id_mhs">
<span<?= $Page->id_mhs->viewAttributes() ?>>
...
<input type="hidden" data-table="mahasiswa" data-field="x_id_mhs" data-hidden="1" name="x_id_mhs" id="x_id_mhs" value="<?= HtmlEncode($Page->id_mhs->FormValue) ?>">
<?php } ?>
</div></div>
    </div>
<?php } ?>
