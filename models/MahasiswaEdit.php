...
class MahasiswaEdit extends Mahasiswa
{
    ...
    // Page object name
    public $PageObjName = "MahasiswaEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("id_mhs") ?? Key(0) ?? Route(2)) !== null) {
                $this->id_mhs->setQueryStringValue($keyValue);
                $this->id_mhs->setOldValue($this->id_mhs->QueryStringValue);
            } elseif (Post("id_mhs") !== null) {
                $this->id_mhs->setFormValue(Post("id_mhs"));
                $this->id_mhs->setOldValue($this->id_mhs->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("id_mhs") ?? Route("id_mhs")) !== null) {
                    $this->id_mhs->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
    ...
    protected function validateForm()
    {
        ...
        if ($this->nim->Required) {
            if (!$this->nim->IsDetailKey && EmptyValue($this->nim->FormValue)) {
                $this->nim->addErrorMessage(str_replace("%s", $this->nim->caption(), $this->nim->RequiredErrorMessage));
            }
        }
        if ($this->nama_mhs->Required) {
            if (!$this->nama_mhs->IsDetailKey && EmptyValue($this->nama_mhs->FormValue)) {
                $this->nama_mhs->addErrorMessage(str_replace("%s", $this->nama_mhs->caption(), $this->nama_mhs->RequiredErrorMessage));
            }
        }
