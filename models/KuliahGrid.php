 ...
 * Page class
 */
class KuliahGrid extends Kuliah
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "KuliahGrid";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fkuliahgrid";
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->id_kuliah->CurrentValue;
    ...
    {
        global $CurrentForm;
        if ($CurrentForm->hasValue("x_id_mk") && $CurrentForm->hasValue("o_id_mk") && $this->id_mk->CurrentValue != $this->id_mk->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_tahun_ajaran") && $CurrentForm->hasValue("o_tahun_ajaran") && $this->tahun_ajaran->CurrentValue != $this->tahun_ajaran->OldValue) {
            return false;
        }
        return true;
    }
    ...
    {
        $this->id_kuliah->clearErrorMessage();
        $this->id_mk->clearErrorMessage();
        $this->tahun_ajaran->clearErrorMessage();
    }
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->id_kuliah->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
        ...
        if ($CurrentForm->hasValue("o_id_mk")) {
            $this->id_mk->setOldValue($CurrentForm->getValue("o_id_mk"));
        }
        ...
        if ($CurrentForm->hasValue("o_tahun_ajaran")) {
            $this->tahun_ajaran->setOldValue($CurrentForm->getValue("o_tahun_ajaran"));
        }
    }
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->getCurrentMasterTable() == "v_mhs") {
            $this->id_mhs->CurrentValue = $this->id_mhs->getSessionValue();
        }
        $conn = $this->getConnection();
        ...
        if ($this->id_mhs->getSessionValue() != "") {
            $rsnew['id_mhs'] = $this->id_mhs->getSessionValue();
        }
